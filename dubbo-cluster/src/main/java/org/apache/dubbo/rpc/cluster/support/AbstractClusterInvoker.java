...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     * @param selected    exclude selected invokers or not 没有成功提供服务的服务提供者列表（但是存活着）
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation,
        ...
        /**
         *如果 stickyInvoker 存在于 invokers 列表中，此时要进行下一项检测 — 检测 selected 中是否包含 stickyInvoker。如果包含的话，
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        //第一步：查找可用的 Invoker，并将其添加到 reselectInvokers 集合中
        ...
        //reselect 从 invokers 列表中查找有效可用的 Invoker，若未能找到，此时再到 selected 列表中继续查找。
        if (selected != null) {
    ...
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        // 调用 doInvoke 进行后续操作，doInvoke是抽象方法，由子类实现
    ...
    protected List<Invoker<T>> list(Invocation invocation) throws RpcException {
        return directory.list(invocation);// 调用 Directory 的 list 方法列举 Invoker
    }
