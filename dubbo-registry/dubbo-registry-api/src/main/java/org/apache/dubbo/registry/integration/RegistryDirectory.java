 ...
 * RegistryDirectory 可收到与当前服务相关的变化。收到变更通知后，RegistryDirectory 可根据配置变更信息
 * 刷新 Invoker 列表
 * 
 */
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    public synchronized void notify(List<URL> urls) {
         ...
         * 
         *  这里分为三种url，分别是服务提供者url、路由url、配置器url,分别使用UrlUtils的isProvider、isRoute、isConfigurator方法进行判断，
         *  然后对this对象的configurators、routerChain变量进行设置，最后刷新 Invoker 列表
         */
    ...
    private List<Invoker<T>> toMergeInvokerList(List<Invoker<T>> invokers) {
        ...
        for (Invoker<T> invoker : invokers) {
            ...
            groupMap.computeIfAbsent(group, k -> new ArrayList<>());//computeIfAbsent 此方法首先判断缓存MAP中是否存在指定key的值，如果不存在，会自动调用mappingFunction(key)计算key的value，然后将key = value放入到缓存Map
            groupMap.get(group).add(invoker);// 缓存 <group, List<Invoker>> 到 groupMap 中
        }

        if (groupMap.size() == 1) {// 如果 groupMap 中仅包含一组键值对，此时直接取出该键值对的值即可
     ...
     * 并将 url 存入到 deleted 列表中。然后再遍历 deleted 列表，并从 oldUrlInvokerMap 中移除相应的 Invoker
     * 
     */
    ...
    public List<Invoker<T>> doList(Invocation invocation) {
        ...
        try {
        	...
        	/**
        	 * 2.7版本之后新增了RouterChain，用于重构路由规则逻辑，新增应用级别路由，Tag路由优化等。
        	 * 针对服务级别的路由，精确到单个服务，避免了无法明确路由规则的问题
        	 */
