...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    protected List<URL> loadRegistries(boolean provider) {
        ...
        //将注册中心配置转化为URL列表，其中若地址为'N/A' 则表示无效注册中心地址，则会跳过。
        //最后返回的URL列表的协议名称为registry，并且会在URL参数registry的值中保存原始的注册中心地址协议名称值
        ...
        if (registries != null && !registries.isEmpty()) {
            ...
            for (RegistryConfig config : registries) {
                ...
                if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    if (!map.containsKey("protocol")) {//若不存在 protocol 参数，缺省默认为 “dubbo”
                        map.put("protocol", "dubbo");
                    }
                    List<URL> urls = UrlUtils.parseURLs(address, map);

                    //循环 urls ，设置 “registry” 和 “protocol” 属性。
                    for (URL url : urls) {
    ...
    /**为了兼容的目的，当注册协议为zk且没明确（explicitly）指定配置中心的时候，使用zk作为配置中心
