...
public abstract class AbstractConfig implements Serializable {
     ...
     * The config id
     */
    protected String id;//配置对象的编号，适用于除了 API 配置之外的三种配置方式
    ...
    protected static void appendParameters(Map<String, String> parameters, Object config, String prefix) {
        ...
        for (Method method : methods) {
            try {
                ...
                if (ClassHelper.isGetter(method)) {//方法是public ，且返回值为基本类型，且没有入参，且以get或is开头，且不等于get、is、getClass、getObject
                    //获取方法上的Parameter注解，例如ApplicationConfig的getName()方法
                    ...
                    if (value != null && str.length() > 0) {
                        ...
                        && method.getReturnType() == Map.class) {//方法名是getParameters，且是public，且没有入参，且返回值是Map
                	//当方法为 #getParameters() 时，通过反射，获得 #getParameters() 的返回值为 map 。将 map 添加到 parameters ，kv 格式为 prefix:entry.key entry.value 。
...
//                            String pre = (prefix != null && prefix.length() > 0 ? prefix + "." : "");
     ...
     * 
     * 当前仅支持覆盖在config类里面明确定义的属性，不支持覆盖自定义的参数
     */
    public void refresh() {
        try {
        	//CompositeConfiguration是配置的集合类，包含了SystemConfig\AppExternalConfig\ExternalConfig\PropertiesConfig
            CompositeConfiguration compositeConfiguration = Environment.getInstance().getConfiguration(getPrefix(), getId());
            InmemoryConfiguration config = new InmemoryConfiguration(getPrefix(), getId());
            config.addProperties(getMetaData());
            if (Environment.getInstance().isConfigCenterFirst()) {
                // The sequence would be: SystemConfiguration -> ExternalConfiguration -> AppExternalConfiguration -> AbstractConfig(对应config) -> PropertiesConfiguration
                compositeConfiguration.addConfiguration(3, config);
            } else {
                // The sequence would be: SystemConfiguration -> AbstractConfig(对应config) -> ExternalConfiguration -> AppExternalConfiguration -> PropertiesConfiguration
            ...
            for (Method method : methods) {
                if (ClassHelper.isSetter(method)) {
                    try {
                        ...
                        //isTypeMatch()方法用来避免重复不正确的更新，例如：在ReferenceConfig中，有两个setGeneric方法，其中方法入参为boolean类型、且value是true或者false的，不执行。
